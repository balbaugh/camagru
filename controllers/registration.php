...
//server-side form validation
    ...
    {
        header("Location: ../sources/registration.html.php?email_error=Email is required!");
        exit();
    }
    else if (empty($empty_username))
    {
        header("Location: ../sources/registration.html.php?username_error=Username is required!");
        exit();
    }
    else if (empty($empty_password))
    {
        header("Location: ../sources/registration.html.php?password_error=Password is required!");
        exit();
    }
    else if (!filter_var($email, FILTER_VALIDATE_EMAIL))
    {
        header("Location: ../sources/registration.html.php?email_error=Invalid email format!");
        exit();
    }
    else if (!preg_match("/^[a-zA-Z0-9]*$/", $username))
    {
        header("Location: ../sources/registration.html.php?username_error=Invalid username!");
        exit();
    }
    else if (strlen($password) < 8)
    {
        header("Location: ../sources/registration.html.php?password_error=Password must be at least 8 characters long!");
        exit();
    }
    else if (!preg_match("/^[_\.0-9a-zA-Z-]+@([0-9a-zA-Z][0-9a-zA-Z-]+\.)+[a-zA-Z]{2,6}$/i", $empty_email)){
        header("Location: ../sources/registration.html.php?email_error=Invalid email address!");
        exit();
    }
    else if(preg_match("/[<>=\{\}\/]/", $empty_username))
    {
        header("Location: ../sources/registration.html.php?username_error=Username cannot contain special characters!");
        exit();
    }
    ...
    try {
        ...
        if ($result == $empty_email) {
            header("Location: ../sources/registration.html.php?email_error=Email already exists!");
            exit();
        }
        if ($result == $empty_username) {
            header("Location: ../sources/registration.html.php?username_error=Username already exists!");
            exit();
        }
        else {
            ...
            $headers = "From: Brian Albaugh <balbaugh@gmail.com>\r\n";
            $headers .= "Reply-To: balbaugh@gmail.com\r\n";
    ...
    header('Location: ../sources/registration.html.php?email_error=An error has occurred. Please try again.');
    exit();
}
